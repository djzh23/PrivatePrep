...
public class MustHaveScorerTests
{
    ...
    [Fact]
    public void RightRequirementWithWrongStatus_StillCountsTowardsRecall()
    {
        var expected = new[] { Item("sprache", "Deutsch C1", "met") };
        var actual = new[] { Item("sprache", "Deutsch C1", "notMet") };
    ...
    [Fact]
    public void ExpectationWithoutStatus_ChecksTheExtractionOnly()
    {
        var expected = new[] { new ExpectedMustHave("sprache", "Deutsch C1") };
        var actual = new[] { Item("sprache", "Deutsch C1", "notMet") };
    ...
    public void Aggregate_SumsAcrossCasesBeforeComputingAccuracy()
    {
        ...
        Assert.Equal(11.0 / 12.0, total.Recall, precision: 6);
    }
}
